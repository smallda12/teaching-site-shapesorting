...
(function () {
  ...
  const 相簿 = (function () {
    ...
    function 開() {
      return new Promise((res) => {
        if (db) return res(db);
        const r = indexedDB.open("形狀分類照片", 1);
  ...
  const 點名 = (function () {
    ...
    async function 匯出() {
      ...
      const 資料 = { 格式: "形狀分類點名名單", 版本: 1, 匯出時間: new Date().toISOString(), 學生: [] };
      for (const s of 名單) {
  ...
  const 延伸 = (function () {
    ...
    function 畫() {
      ...
      /* 🆕 2026-09-15〈形狀分類〉第一次出現非 YouTube 來源（HKIEd 網頁播放器）：
         data.js 該筆沒有 "id"、改用 "連結" 存外部網址。原本這裡假設每一筆都有 id，
         直接組 YouTube 網址與縮圖——沒有前例可循，這裡就地加上「有 id 就走 YouTube
         縮圖／有 連結 就走通用外部連結卡（無縮圖圖示、直接連到該網址）」的判斷，
         舊單元的資料都有 id，行為不變。 */
      $("#延伸內容").innerHTML = 排序.map((k) => `
        <div class="延伸組">
          ...
          <div class="延伸牆">
            ${組.get(k).map((v) => {
              const 是YT = !!v.id;
              const 網址 = 是YT ? `https://www.youtube.com/watch?v=${v.id}` : v.連結;
              const 縮圖 = 是YT
                ...
                : "";
              return `
              <a class="延伸卡${是YT ? "" : " 外部連結"}" href="${網址}"
                 target="_blank" rel="noopener noreferrer">
                <div class="延伸縮圖${是YT ? "" : " 外部連結"}">
                ...
                <div class="延伸文字">
                  <div class="延伸標題">${v.標題}${是YT ? "" : "　<span class=\"延伸外連標\">🔗 網頁連結（非 YouTube）</span>"}</div>
